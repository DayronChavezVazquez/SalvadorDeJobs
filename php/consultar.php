...
<div class="filters-container">
    <div class="filter-group">
        ...
        <input type="text" id="search-nombre" class="search-input autocomplete" name="q" value="<?= htmlspecialchars((string)$q, ENT_QUOTES, 'UTF-8') ?>" placeholder="Escribe el nombre..." autocomplete="off">
        <div id="autocomplete-nombre" class="autocomplete-results"></div>
    </div>
    <div class="filter-group">
        <label>Buscar por teléfono:</label>
        <input type="text" id="search-telefono" class="search-input autocomplete" name="q_tel" value="<?= htmlspecialchars((string)$q_tel, ENT_QUOTES, 'UTF-8') ?>" placeholder="Escribe el teléfono..." autocomplete="off">
        <div id="autocomplete-telefono" class="autocomplete-results"></div>
    </div>
    <?php if ($q !== '' || $q_tel !== ''): ?>
        <a href="?page=consultar" class="btn btn-link" id="btn-limpiar-filtros">Limpiar filtros</a>
    <?php endif; ?>
</div>
...
<div class="paginacion">
    <?php for ($i = 1; $i <= $total_paginas; $i++): ?>
        <a href="?page=consultar&p=<?= $i ?><?= $q !== '' ? '&q=' . urlencode($q) : '' ?><?= $q_tel !== '' ? ($q !== '' ? '&' : '') . 'q_tel=' . urlencode($q_tel) : '' ?>"><?= $i ?></a>
    <?php endfor; ?>
</div>
...
<div id="modalEliminar">
    <div class="modal-content">
        ...
        <p style="font-weight: bold; color: var(--text); margin-bottom: 20px;">Institución: <span id="nombre-eliminar"></span></p>
        <p style="color: #b91c1c; font-size: 13px;">Esta acción no se puede deshacer.</p>
